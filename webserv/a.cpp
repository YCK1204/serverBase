...
int main()
{
    ...
    map<int, time_t> last_active_times; // map for client socket:last active time
    ...
    while (1)
    {
        ...
        for (int i = 0; i < new_events; ++i)
        {
            ...
            if (curr_event->flags & EV_ERROR)
            {
                ...
                else
                {
                    ...
                    last_active_times.erase(curr_event->ident);
                }
            }
            else if (curr_event->filter == EVFILT_READ)
            {
                if (curr_event->ident == server_socket)
                {
                    ...
                    last_active_times[client_socket] = time(NULL);
                }
                else if (clients.find(curr_event->ident) != clients.end())
                {
                    ...
                    else
                    {
                        ...
                        last_active_times[curr_event->ident] = time(NULL);
                    }
                }
            }
        ...
        // Check for idle clients and disconnect
        time_t current_time = time(NULL);
        for (map<int, time_t>::iterator it = last_active_times.begin(); it != last_active_times.end();)
        {
            if (current_time - it->second >= 6)
            {
                disconnect_client(it->first, clients);
                it = last_active_times.erase(it);
            }
            else
            {
                ++it;
